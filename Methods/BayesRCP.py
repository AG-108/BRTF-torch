...
def BayesRCP(Y, init=INIT_ML, maxRank=None, dimRed=1, initVar=1, updateHyper=UPDATEHYPER_ON,
             maxIters=100, tol=1e-5, predVar=PREDVAR_DOES_NOT_COMPUTE, verbose=VERBOSE_TEXT):
    """
    ...
    :param tol:             - lower band change tolerance for convergence detection (default: 1e-5)
    ...
    for it in range(maxIters):
        ...
        items[10] = (torch.lgamma(a_alphaN) - (a_alphaN - 1) * torch.special.psi(a_alphaN)
        ...
        comPower = (Zall.T.matmul(Zall)).diag()
        # comPower = (Zall.mH.matmul(Zall)).diag()
        print('comPower: ', comPower)
        comTol = dimY.sum() * np.finfo(float(Zall.norm("fro"))).eps
        print('comTol: ', comTol)
        rankest = (comPower > comTol).sum()
        print('rankest: ', rankest)
        print('rankest.max(): ', rankest.max())
